...
class ModelBasedSolver(Solver):
  ...
  def propose(
    ...
    all_candidates_df['sequence'] = all_candidates_df['sequence'].apply(tuple)
  ...
  def _get_model_predictions(self, x):
    if self._flatten_inputs:
      y_pred = self._model.predict(utils.one_hot_and_flatten(x, self._vocab_size))
    else:
      y_pred = self._model.predict(utils.onehot(x, num_classes=self._vocab_size))
    return y_pred
